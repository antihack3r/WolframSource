...
public class Block {
    ...
    public boolean isFullCube(IBlockState state) {
        if (Xray.getInstance().isEnabled()) {
            return Xray.getInstance().isXrayBlock(this);
        }
        return true;
    }
    ...
    public int getPackedLightmapCoords(IBlockState state, IBlockAccess source, BlockPos pos) {
        if (Xray.getInstance().isEnabled() || Wolfram.getWolfram().moduleManager.isEnabled("fullbright")) {
            return 1000;
        }
    ...
    public void dropBlockAsItemWithChance(World worldIn, BlockPos pos, IBlockState state, float chance, int fortune) {
        if (!worldIn.isRemote) {
            ...
            while (j < i) {
                Item item;
                if (worldIn.rand.nextFloat() <= chance && (item = this.getItemDropped(state, worldIn.rand, fortune)) != Items.field_190931_a) {
