...
extends BlockContainer {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing heldItem, float side, float hitX, float hitY) {
        ...
        if (tileentity instanceof TileEntityShulkerBox) {
            ...
            boolean flag = ((TileEntityShulkerBox)tileentity).func_190591_p() == TileEntityShulkerBox.AnimationStatus.CLOSED ? !worldIn.collidesWithAnyBlock((axisalignedbb = FULL_BLOCK_AABB.addCoord(0.5f * (float)enumfacing.getFrontOffsetX(), 0.5f * (float)enumfacing.getFrontOffsetY(), 0.5f * (float)enumfacing.getFrontOffsetZ()).func_191195_a(enumfacing.getFrontOffsetX(), enumfacing.getFrontOffsetY(), enumfacing.getFrontOffsetZ())).offset(pos.offset(enumfacing))) : true;
            if (flag) {
