...
import net.wolframclient.compatibility.CPacketNewKeepAlive;
import net.wolframclient.compatibility.SPacketNewKeepAlive;
...
implements INetHandlerPlayClient {
    ...
    public void handleJoinGame(SPacketJoinGame packetIn) {
        ...
        Wolfram.getWolfram().onWorldLoad();
    }
    ...
    public void handleEntityVelocity(SPacketEntityVelocity packetIn) {
        ...
        if (entity != null) {
            if (Wolfram.getWolfram().moduleManager.getModule("velocity").isEnabled()) {
                float hor = Wolfram.getWolfram().storageManager.moduleSettings.getFloat("horizontal_velocity");
                float vert = Wolfram.getWolfram().storageManager.moduleSettings.getFloat("vertical_velocity");
                entity.setVelocity((double)packetIn.getMotionX() / 8000.0 * (double)hor, (double)packetIn.getMotionY() / 8000.0 * (double)vert, (double)packetIn.getMotionZ() / 8000.0 * (double)hor);
            } else {
    ...
    public void handleDestroyEntities(SPacketDestroyEntities packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        int i = 0;
        while (i < packetIn.getEntityIDs().length) {
    ...
    public void handleMultiBlockChange(SPacketMultiBlockChange packetIn) {
        ...
        int n2 = 0;
        while (n2 < n) {
            SPacketMultiBlockChange.BlockUpdateData spacketmultiblockchange$blockupdatedata = blockUpdateDataArray[n2];
    ...
    public void onDisconnect(ITextComponent reason) {
        ...
        Wolfram.getWolfram().onDisconnect();
    }

    public void sendPacketBypass(Packet<?> packetIn) {
        this.netManager.sendPacket(packetIn);
    }

    public void sendPacket(Packet<?> packetIn) {
        AddPacketToQueueEvent event = new AddPacketToQueueEvent(packetIn);
    ...
    public void handleSetSlot(SPacketSetSlot packetIn) {
        ...
        if (packetIn.getWindowId() == -1) {
            ...
            if (packetIn.getWindowId() == 0 && packetIn.getSlot() >= 36 && i < 45) {
                ...
                if (!itemstack.func_190926_b() && ((itemstack1 = entityplayer.inventoryContainer.getSlot(i).getStack()).func_190926_b() || itemstack1.func_190916_E() < itemstack.func_190916_E())) {
    ...
    public void handleTabComplete(SPacketTabComplete packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        Object[] astring = packetIn.getMatches();
        Arrays.sort(astring);
        if (this.gameController.currentScreen instanceof ITabCompleter) {
            ((ITabCompleter)((Object)this.gameController.currentScreen)).setCompletions((String[])astring);
        }
    }
    ...
    private boolean validateResourcePackUrl(String p_189688_1_) {
        block4: {
            try {
                ...
                if (flag && (p_189688_1_.contains("..") || !p_189688_1_.endsWith("/resources.zip"))) break block4;
                return true;
            }
    ...
    public void handleCustomPayload(SPacketCustomPayload packetIn) {
        ...
        if ("MC|TrList".equals(packetIn.getChannelName())) {
            ...
            this.gameController.player.setServerBrand(packetIn.getBufferData().readStringFromBuffer(Short.MAX_VALUE));
            return;
            ...
            String s = packetbuffer3.readStringFromBuffer(Short.MAX_VALUE);
    ...
    public void handleParticles(SPacketParticles packetIn) {
        ...
        if (packetIn.getParticleCount() == 0) {
            ...
            int k = 0;
            while (k < packetIn.getParticleCount()) {
