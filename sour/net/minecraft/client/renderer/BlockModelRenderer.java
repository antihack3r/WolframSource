...
public class BlockModelRenderer {
    ...
    public boolean renderModel(IBlockAccess worldIn, IBakedModel modelIn, IBlockState stateIn, BlockPos posIn, BufferBuilder buffer, boolean checkSides, long rand) {
        ...
        boolean flag;
        block7: {
            flag = Minecraft.isAmbientOcclusionEnabled() && stateIn.getLightValue() == 0 && modelIn.isAmbientOcclusion();
            try {
                if (!Xray.getInstance().isEnabled() || Xray.getInstance().isXrayBlock(stateIn.getBlock())) break block7;
                return false;
            }
    ...
    public boolean renderModelSmooth(IBlockAccess worldIn, IBakedModel modelIn, IBlockState stateIn, BlockPos posIn, BufferBuilder buffer, boolean checkSides, long rand) {
        ...
        if (!list1.isEmpty()) {
            list1 = BlockModelCustomizer.getRenderQuads(list1, worldIn, stateIn, posIn, null, rand, renderenv);
