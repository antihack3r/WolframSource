...
implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            Vec3d vec3d3 = null;
            float f = 1.0f;
            List<Entity> list = this.mc.world.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec3d1.xCoord * d0, vec3d1.yCoord * d0, vec3d1.zCoord * d0).expand(1.0, 1.0, 1.0), (Predicate<? super Entity>)Predicates.and(EntitySelectors.NOT_SPECTATING, (Predicate)new Predicate<Entity>(){
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (Wolfram.getWolfram().moduleManager.getModule("antihurtcam").isEnabled()) {
            return;
        }
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPlayerSleeping()) {
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                while (i < 8) {
                    ...
                    float f4 = (i >> 1 & 1) * 2 - 1;
                    float f5 = (i >> 2 & 1) * 2 - 1;
                    if ((raytraceresult = this.mc.world.rayTraceBlocks(new Vec3d(d0 + (double)(f3 *= 0.1f), d1 + (double)(f4 *= 0.1f), d2 + (double)(f5 *= 0.1f)), new Vec3d(d0 - d4 + (double)f3 + (double)f5, d1 - d6 + (double)f4, d2 - d5 + (double)f5))) != null && !Wolfram.getWolfram().moduleManager.isEnabled("cameranoclip") && (d7 = raytraceresult.hitVec.distanceTo(new Vec3d(d0, d1, d2))) < d3) {
                        d3 = d7;
                    }
                    ++i;
                }
    ...
    public void setupCameraTransform(float partialTicks, int pass) {
        ...
        if (this.mc.gameSettings.viewBobbing && !Wolfram.getWolfram().moduleManager.isEnabled("tracers")) {
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime) {
        Dispatcher.call(new RenderEvent(partialTicks));
        this.frameInit();
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (this.mc.world == null) {
                ...
                this.mc.mcProfiler.endSection();
                GL11.glPushMatrix();
                Wolfram.getWolfram().guiManager.render();
                GL11.glPopMatrix();
            }
            if (this.mc.currentScreen != null) {
    ...
    public void renderWorld(float partialTicks, long finishTimeNano) {
        ...
        if (!Wolfram.getWolfram().moduleManager.isEnabled("cavefinder")) {
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        ...
        if (!Config.isShaders()) {
            Dispatcher.call(new WorldRenderEvent());
        }
    ...
    private void updateFogColor(float partialTicks) {
        ...
        if (this.cloudFog) {
            ...
            Vec3d vec3d6 = CustomColors.getUnderwaterColor(this.mc.world, this.mc.getRenderViewEntity().posX, this.mc.getRenderViewEntity().posY + 1.0, this.mc.getRenderViewEntity().posZ);
            if (vec3d6 != null) {
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPotionActive(MobEffects.BLINDNESS) && !Wolfram.getWolfram().moduleManager.getModule("noblind").isEnabled()) {
    ...
    private void setupFog(int startCoords, float partialTicks) {
        ...
        } else if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPotionActive(MobEffects.BLINDNESS) && !Wolfram.getWolfram().moduleManager.getModule("noblind").isEnabled()) {
            float f2 = 5.0f;
            int i = ((EntityLivingBase)entity).getActivePotionEffect(MobEffects.BLINDNESS).getDuration();
            if (i < 20) {
    ...
    private void frameInit() {
        ...
        if (world != null) {
            if (Config.getNewRelease() != null) {
                ...
                String s1 = String.valueOf(s) + " " + Config.getNewRelease();
