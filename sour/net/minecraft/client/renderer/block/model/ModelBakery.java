...
public class ModelBakery {
    ...
    protected void loadBlock(BlockStateMapper p_loadBlock_1_, Block p_loadBlock_2_, final ResourceLocation p_loadBlock_3_) {
        ...
        if (modelblockdefinition.hasMultipartData()) {
            HashSet collection = Sets.newHashSet(map.values());
            modelblockdefinition.getMultipartData().setStateContainer(p_loadBlock_2_.getBlockState());
            ArrayList collection1 = this.multipartVariantMap.get(modelblockdefinition);
            if (collection1 == null) {
                collection1 = Lists.newArrayList();
            }
            collection1.addAll(Lists.newArrayList((Iterable)Iterables.filter((Iterable)collection, (Predicate)new Predicate<ModelResourceLocation>(){
    ...
    private void loadVariantItemModels() {
        this.variants.put(MODEL_MISSING, new VariantList(Lists.newArrayList((Object[])new Variant[]{new Variant(new ResourceLocation(MODEL_MISSING.getResourcePath()), ModelRotation.X0_Y0, false, 1)})));
        this.func_191401_d();
    ...
    public void loadItemModel(String variantName, ResourceLocation location, ResourceLocation itemName) {
        ...
        if (this.models.get(location) == null) {
            try {
                ...
                LOGGER.warn("Unable to load item model: '{}' for item: '{}'", (Object)location, (Object)itemName);
                LOGGER.warn(String.valueOf(exception1.getClass().getName()) + ": " + exception1.getMessage());
            }
        }
    }
    ...
    private Set<ResourceLocation> getVariantsTextureLocations() {
        ...
        ArrayList list = Lists.newArrayList(this.variants.keySet());
        ...
        for (ModelBlockDefinition modelblockdefinition : this.multipartVariantMap.keySet()) {
            for (VariantList variantlist1 : modelblockdefinition.getMultipartData().getVariants()) {
                for (Variant variant1 : variantlist1.getVariantList()) {
                    ...
                    if (modelblock1 == null) {
                        LOGGER.warn("Missing model for: {}", (Object)Block.REGISTRY.getNameForObject(modelblockdefinition.getMultipartData().getStateContainer().getBlock()));
                        continue;
                    }
    ...
    private void loadModels() {
        ...
        while (!deque.isEmpty()) {
            ...
            try {
                ...
                LOGGER.warn("In parent chain: {}; unable to load model: '{}'", (Object)JOINER.join(this.getParentPath(resourcelocation1)), (Object)resourcelocation1);
            }
    ...
    public static void registerItemVariants(Item p_registerItemVariants_0_, ResourceLocation ... p_registerItemVariants_1_) {
        ...
        ResourceLocation[] resourceLocationArray = p_registerItemVariants_1_;
        int n = p_registerItemVariants_1_.length;
        int n2 = 0;
        while (n2 < n) {
