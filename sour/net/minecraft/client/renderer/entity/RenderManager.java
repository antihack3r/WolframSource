...
public class RenderManager {
    ...
    public <T extends Entity> Render<T> getEntityClassRenderObject(Class entityClass) {
    ...
    public void renderEntityStatic(Entity entityIn, float partialTicks, boolean p_188388_3_) {
        ...
        this.doRenderEntity(entityIn, d0 - renderPosX, d1 - renderPosY, d2 - renderPosZ, f, partialTicks, p_188388_3_);
    }
    ...
    public void doRenderEntity(Entity entityIn, double x, double y, double z, float yaw, float partialTicks, boolean p_188391_10_) {
        block14: {
            ...
            try {
                ...
                if (render == null || this.renderEngine == null) break block14;
                try {
                    ...
                    if (Wolfram.getWolfram().moduleManager.getModule("wallhack").isEnabled()) {
                        if (Wolfram.getWolfram().storageManager.moduleSettings.getBoolean("wallhack_overlay")) {
                ...
                if (!this.debugBoundingBox || entityIn.isInvisible() || p_188391_10_ || Minecraft.getMinecraft().isReducedDebug()) break block14;
                try {
    ...
    private void renderDebugBoundingBox(Entity entityIn, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        if (entityIn instanceof EntityLivingBase) {
            float f1 = 0.01f;
            RenderGlobal.drawBoundingBox(x - (double)f, y + (double)entityIn.getEyeHeight() - (double)0.01f, z - (double)f, x + (double)f, y + (double)entityIn.getEyeHeight() + (double)0.01f, z + (double)f, 1.0f, 0.0f, 0.0f, 1.0f);
        }
        ...
        bufferbuilder.pos(x + vec3d.xCoord * 2.0, y + (double)entityIn.getEyeHeight() + vec3d.yCoord * 2.0, z + vec3d.zCoord * 2.0).color(0, 0, 255, 255).endVertex();
        tessellator.draw();
