...
extends EntityLivingBase {
    ...
    public boolean isEntityInsideOpaqueBlock() {
        if (Wolfram.getWolfram().moduleManager.isEnabled("phase")) {
            return false;
        }
    ...
    protected int getExperiencePoints(EntityPlayer player) {
        if (this.experienceValue > 0) {
            ...
            int j = 0;
            while (j < this.inventoryArmor.size()) {
            ...
            int k = 0;
            while (k < this.inventoryHands.size()) {
    ...
    public void writeEntityToNBT(NBTTagCompound compound) {
        ...
        float[] fArray = this.inventoryArmorDropChances;
        int n = this.inventoryArmorDropChances.length;
        int nbttagcompound1 = 0;
        while (nbttagcompound1 < n) {
            float f = fArray[nbttagcompound1];
            nbttaglist2.appendTag(new NBTTagFloat(f));
            ++nbttagcompound1;
        }
        compound.setTag("ArmorDropChances", nbttaglist2);
        NBTTagList nbttaglist3 = new NBTTagList();
        float[] fArray2 = this.inventoryHandsDropChances;
        int n2 = this.inventoryHandsDropChances.length;
        n = 0;
        while (n < n2) {
            float f1 = fArray2[n];
    ...
    public void readEntityFromNBT(NBTTagCompound compound) {
        ...
        if (compound.hasKey("ArmorItems", 9)) {
            ...
            int i = 0;
            while (i < this.inventoryArmor.size()) {
        ...
        if (compound.hasKey("HandItems", 9)) {
            ...
            int j = 0;
            while (j < this.inventoryHands.size()) {
    ...
    public void onLivingUpdate() {
        ...
        if (!this.world.isRemote && this.canPickUpLoot() && !this.dead && this.world.getGameRules().getBoolean("mobGriefing")) {
            for (EntityItem entityitem : this.world.getEntitiesWithinAABB(EntityItem.class, this.getEntityBoundingBox().expand(1.0, 0.0, 1.0))) {
                if (entityitem.isDead || entityitem.getEntityItem().func_190926_b() || entityitem.cannotPickup()) continue;
        ...
        if (this.rand.nextFloat() < 0.15f * difficulty.getClampedAdditionalDifficulty()) {
            ...
            while (n2 < n) {
                ...
                if (entityequipmentslot.getSlotType() == EntityEquipmentSlot.Type.ARMOR) {
                    ...
                    flag = false;
                    if (itemstack.func_190926_b() && (item = EntityLiving.getArmorByChance(entityequipmentslot, i)) != null) {
    ...
    protected void setEnchantmentBasedOnDifficulty(DifficultyInstance difficulty) {
        ...
        while (n2 < n) {
            ItemStack itemstack;
            EntityEquipmentSlot entityequipmentslot = entityEquipmentSlotArray[n2];
            if (entityequipmentslot.getSlotType() == EntityEquipmentSlot.Type.ARMOR && !(itemstack = this.getItemStackFromSlot(entityequipmentslot)).func_190926_b() && this.rand.nextFloat() < 0.5f * f) {
