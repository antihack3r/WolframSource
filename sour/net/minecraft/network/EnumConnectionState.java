...
import net.wolframclient.compatibility.CPacketSomethingNew;
import net.wolframclient.compatibility.SPacketNewKeepAlive;
import net.wolframclient.compatibility.SPacketSomethingNew;
...
public enum EnumConnectionState {
    ...
    PLAY(0){
        {
            ...
            if (Wolfram.getWolfram().getClientSettings().getInt("mc112x_compatibility") == 0) {
    ...
    static {
        ...
        EnumConnectionState[] enumConnectionStateArray = EnumConnectionState.values();
        int n = enumConnectionStateArray.length;
        int n2 = 0;
        while (n2 < n) {
            EnumConnectionState enumconnectionstate = enumConnectionStateArray[n2];
            ...
            for (EnumPacketDirection enumpacketdirection : enumconnectionstate.directionMaps.keySet()) {
                for (Class oclass : enumconnectionstate.directionMaps.get((Object)enumpacketdirection).values()) {
                    if (STATES_BY_CLASS.containsKey(oclass) && STATES_BY_CLASS.get(oclass) != enumconnectionstate) {
                        throw new Error("Packet " + oclass + " is already assigned to protocol " + (Object)((Object)STATES_BY_CLASS.get(oclass)) + " - can't reassign to " + (Object)((Object)enumconnectionstate));
                    }
                    try {
                        oclass.newInstance();
                    }
    ...
    protected EnumConnectionState registerPacket(EnumPacketDirection direction, Class<? extends Packet<?>> packetClass) {
        HashBiMap bimap = this.directionMaps.get((Object)direction);
        if (bimap == null) {
            bimap = HashBiMap.create();
            this.directionMaps.put(direction, (BiMap<Integer, Class<Packet<?>>>)bimap);
        }
    ...
    /* synthetic */ EnumConnectionState(String string, int n, int n2, EnumConnectionState enumConnectionState) {
        this(n2);
    }
}
