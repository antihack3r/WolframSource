...
public class ConnectedTextures {
    ...
    private static BakedQuad[] getConnectedTextureMultiPass(IBlockAccess p_getConnectedTextureMultiPass_0_, IBlockState p_getConnectedTextureMultiPass_1_, BlockPos p_getConnectedTextureMultiPass_2_, EnumFacing p_getConnectedTextureMultiPass_3_, BakedQuad p_getConnectedTextureMultiPass_4_, RenderEnv p_getConnectedTextureMultiPass_5_) {
        ...
        while (i < list.size()) {
            ...
            int j = 0;
            while (j < 3) {
                BakedQuad[] abakedquad1 = ConnectedTextures.getConnectedTextureSingle(p_getConnectedTextureMultiPass_0_, p_getConnectedTextureMultiPass_1_, p_getConnectedTextureMultiPass_2_, p_getConnectedTextureMultiPass_3_, bakedquad1, false, j + 1, p_getConnectedTextureMultiPass_5_);
        ...
        while (i < astring.length) {
            ...
            try {
                ...
                if (inputstream == null) {
                    ...
                    if (connectedproperties.isValid((String)s)) {
    ...
    private static boolean detectMultipass() {
        ...
        int k = 0;
        while (k < blockProperties.length) {
        ...
        int j = 0;
        while (j < aconnectedproperties1.length) {
    ...
    private static String[] getDefaultCtmPaths() {
        ...
        while (i < astring.length) {
            ...
            if (Config.isFromDefaultResourcePack(new ResourceLocation("textures/blocks/glass_" + s1 + ".png"))) {
                list.add(String.valueOf(s) + i + "_glass_" + s1 + "/glass_" + s1 + ".properties");
                list.add(String.valueOf(s) + i + "_glass_" + s1 + "/glass_pane_" + s1 + ".properties");
            }
            ++i;
        }
