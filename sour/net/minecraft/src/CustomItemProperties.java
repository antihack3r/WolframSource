...
import net.minecraft.src.ParserEnchantmentId;
...
public class CustomItemProperties {
    ...
    public CustomItemProperties(Properties p_i30_1_, String p_i30_2_) {
        this.name = CustomItemProperties.parseName(p_i30_2_);
        this.basePath = CustomItemProperties.parseBasePath(p_i30_2_);
        ...
        this.mapModels = CustomItemProperties.parseModels(p_i30_1_, this.basePath);
        this.model = CustomItemProperties.parseModel(p_i30_1_.getProperty("model"), p_i30_2_, this.basePath, this.type, this.mapModels);
        this.mapTextures = CustomItemProperties.parseTextures(p_i30_1_, this.basePath);
        boolean flag = this.mapModels == null && this.model == null;
        this.texture = CustomItemProperties.parseTexture(p_i30_1_.getProperty("texture"), p_i30_1_.getProperty("tile"), p_i30_1_.getProperty("source"), p_i30_2_, this.basePath, this.type, this.mapTextures, flag);
                ...
                block12: {
                    ...
                    block11: {
                        ...
                        block10: {
                            ...
                            if (j < 0) break block10;
                            set.add(new Integer(j));
                            break block9;
                        }
                        if (!s.contains("-") || (astring1 = Config.tokenize(s, "-")).length != 2) break block11;
                        int k = Config.parseInt(astring1[0], -1);
                        int l = Config.parseInt(astring1[1], -1);
                        if (k < 0 || l < 0) break block11;
                        int i1 = Math.min(k, l);
                        j1 = Math.max(k, l);
                        break block12;
                    }
    ...
    private static Map getMatchingProperties(Properties p_getMatchingProperties_0_, String p_getMatchingProperties_1_) {
        ...
        for (Object s0 : p_getMatchingProperties_0_.keySet()) {
            ...
            if (!s.startsWith(p_getMatchingProperties_1_)) continue;
            map.put(s, s1);
        }
        return map;
    }
    ...
    private ResourceLocation getTextureLocation(String p_getTextureLocation_1_) {
        ...
        if (!(flag = Config.hasResource(resourcelocation1 = new ResourceLocation(s, s2 = String.valueOf(s1) + ".png")))) {
    ...
    private static ModelBlock makeModelBlock(String[] p_makeModelBlock_0_) {
        ...
        int i = 0;
        while (i < p_makeModelBlock_0_.length) {
    ...
    public IBakedModel getBakedModel(ResourceLocation p_getBakedModel_1_, boolean p_getBakedModel_2_) {
        ...
        if (p_getBakedModel_1_ != null && map != null && (ibakedmodel1 = map.get(s = p_getBakedModel_1_.getResourcePath())) != null) {
            return ibakedmodel1;
        }
        return ibakedmodel;
    }
