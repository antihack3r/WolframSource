...
public class DynamicLight {
    ...
    public void update(RenderGlobal p_update_1_) {
        ...
        if (Math.abs(d2) > d5 || Math.abs(d3) > d5 || Math.abs(d4) > d5 || this.lastLightLevel != j) {
            ...
            WorldClient world = p_update_1_.getWorld();
            if (world != null) {
            ...
            if (j > 0) {
                EnumFacing enumfacing2 = (MathHelper.floor(d6) & 0xF) >= 8 ? EnumFacing.EAST : EnumFacing.WEST;
                EnumFacing enumfacing = (MathHelper.floor(d0) & 0xF) >= 8 ? EnumFacing.UP : EnumFacing.DOWN;
                EnumFacing enumfacing1 = (MathHelper.floor(d1) & 0xF) >= 8 ? EnumFacing.SOUTH : EnumFacing.NORTH;
