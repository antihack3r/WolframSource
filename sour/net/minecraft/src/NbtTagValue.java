...
public class NbtTagValue {
    ...
    public NbtTagValue(String p_i66_1_, String p_i66_2_) {
        ...
        if (p_i66_2_.startsWith(PREFIX_PATTERN)) {
            this.type = 1;
            p_i66_2_ = p_i66_2_.substring(PREFIX_PATTERN.length());
        } else if (p_i66_2_.startsWith(PREFIX_IPATTERN)) {
            this.type = 2;
            p_i66_2_ = p_i66_2_.substring(PREFIX_IPATTERN.length()).toLowerCase();
        } else if (p_i66_2_.startsWith(PREFIX_REGEX)) {
            this.type = 3;
            p_i66_2_ = p_i66_2_.substring(PREFIX_REGEX.length());
        } else if (p_i66_2_.startsWith(PREFIX_IREGEX)) {
            this.type = 4;
            p_i66_2_ = p_i66_2_.substring(PREFIX_IREGEX.length()).toLowerCase();
        } else {
            this.type = 0;
        }
        p_i66_2_ = StringEscapeUtils.unescapeJava((String)p_i66_2_);
    ...
    public boolean matchesCompound(NBTTagCompound p_matchesCompound_1_) {
        ...
        while (i < this.parents.length) {
            ...
            if ((nbtbase = NbtTagValue.getChildTag(nbtbase, s)) == null) {
                return false;
            }
            ++i;
        }
    ...
    private boolean matches(NBTBase p_matches_1_) {
        ...
        String s = NbtTagValue.getNbtString(p_matches_1_, this.valueFormat);
        if (s == null) {
            return false;
        }
        switch (this.type) {
