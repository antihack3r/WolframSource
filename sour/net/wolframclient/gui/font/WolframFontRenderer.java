...
import net.wolframclient.gui.font.WolframFont;
import org.lwjgl.opengl.GL11;

public class WolframFontRenderer
extends FontRenderer {
    public final Random fontRandom = new Random();
    private final Color[] customColorCodes = new Color[256];
    private final int[] colorCode = new int[32];
    private WolframFont font;
    private WolframFont boldFont;
    private WolframFont italicFont;
    private WolframFont boldItalicFont;
    private String colorcodeIdentifiers = "0123456789abcdefklmnor";
    private boolean bidi;

    public WolframFontRenderer(Font font, boolean antiAlias, int charOffset) {
    ...
    public int drawString(String text, float x, float y, int color, boolean shadow) {
        ...
        if (text.contains("\u00a7")) {
            ...
            WolframFont currentFont = this.font;
            int width = 0;
            ...
            while (index < parts.length) {
                if (parts[index].length() > 0) {
                    if (index == 0) {
                        ...
                        if (strikethrough) {
                            this.drawLine(width, currentFont.getStringHeight(words) / 2 + 1, width + currentFont.getStringWidth(words), currentFont.getStringHeight(words) / 2 + 1, 1.0f);
                        }
                        if (underline) {
                            this.drawLine(width, currentFont.getStringHeight(words), width + currentFont.getStringWidth(words), currentFont.getStringHeight(words), 1.0f);
                        }
    ...
    private String toRandom(WolframFont font, String text) {
        String newText = "";
    ...
    public static String getFormatFromString(String p_78282_0_) {
        ...
        while ((var2 = p_78282_0_.indexOf(167, var2 + 1)) != -1) {
            ...
            if (WolframFontRenderer.isFormatColor(var4)) {
                var1 = "\u00a7" + var4;
                continue;
            }
            if (!WolframFontRenderer.isFormatSpecial(var4)) continue;
            ...
            switch (var8) {
                ...
                case '\u00a7': {
                    ...
                    if ((var9 = str.charAt(++var5)) != 'l' && var9 != 'L') {
                        if (var9 != 'r' && var9 != 'R' && !WolframFontRenderer.isFormatColor(var9)) break;
                        var7 = false;
                        break;
                    }
                    var7 = true;
    ...
    public void setFont(Font font, boolean antiAlias, int charOffset) {
        WolframFontRenderer wolframFontRenderer = this;
        synchronized (wolframFontRenderer) {
            this.font = new WolframFont(font, antiAlias, charOffset);
            this.boldFont = new WolframFont(font.deriveFont(1), antiAlias, charOffset);
            this.italicFont = new WolframFont(font.deriveFont(2), antiAlias, charOffset);
            this.boldItalicFont = new WolframFont(font.deriveFont(3), antiAlias, charOffset);
            this.FONT_HEIGHT = this.getHeight();
        }
    }
        ...
        if ((double)this.getStringWidth(text) > width) {
            ...
            while (n2 < n) {
                ...
                if ((double)this.getStringWidth(stringBuilder.append(word).append(" ").toString()) < width) {
                    ...
                    currentWord = lastColorCode == '\uffffffff' ? String.valueOf(word) + " " : "\u00a7" + lastColorCode + word + " ";
                }
                ++n2;
            }
            if (!currentWord.equals("")) {
                if ((double)this.getStringWidth(currentWord) < width) {
                    finalWords.add(lastColorCode == '\uffffffff' ? String.valueOf(currentWord) + " " : "\u00a7" + lastColorCode + currentWord + " ");
                    currentWord = "";
                } else {
    ...
    public List<String> formatString(String s, double width) {
        ...
        while (i < s.toCharArray().length) {
            ...
            if ((double)this.getStringWidth(stringBuilder.append(c).toString()) < width) {
                ...
                finalWords.add(currentWord);
                currentWord = lastColorCode == '\uffffffff' ? String.valueOf(c) : "\u00a7" + lastColorCode + String.valueOf(c);
            }
            ++i;
        }
        if (!currentWord.equals("")) {
    ...
    protected String wrapFormattedStringToWidth(String str, int wrapWidth) {
        ...
        String var7 = String.valueOf(WolframFontRenderer.getFormatFromString(var4)) + str.substring(var3 + (var6 ? 1 : 0));
